...
(*||*)
Notation "B+" := (BaseP) : ty_scope .
Notation "B-" := (BaseN) : ty_scope .
...
| Var {Γ A} : Γ ∋ A -> whn Γ A
| ArrR {Γ A B} : state (Γ ▶ t+ A ▶ t- B) -> whn Γ (t+ (A → B))
| ArrL {Γ A B} : whn Γ (t+ A) -> whn Γ (t- B) -> whn Γ (t- (A → B))
...
with w_rename {Γ Δ} : Γ ⊆ Δ -> whn Γ ⇒ᵢ whn Δ :=
  ...
  w_rename f _ (ArrR c)     := ArrR (s_rename (r_shift2 f) c) ;
  w_rename f _ (ArrL v k) := ArrL (w_rename f _ v) (w_rename f _ k) ;
...
Definition a_shift2 {Γ Δ} [y z] (a : Γ =[val]> Δ) : (Γ ▶ y ▶ z) =[val]> (Δ ▶ y ▶ z) :=
  s_append (s_append (fun _ i => v_shift2 _ (a _ i)) (s_var _ (pop (top)))) (s_var _ top).
...
Definition w_shift_n {Γ} ts : whn Γ ⇒ᵢ whn (Γ +▶ ts).
  induction ts; intros ? v; [ exact v | exact (w_shift _ (IHts _ v)) ].
Defined.
...
Definition App' {Γ a b} (f : term Γ (t+ (a → b))) (x : whn Γ a) : term Γ (t+ b) :=
...
(*
Definition Ram' {Γ a b} (u : term (Γ ▶ t+ a) (t+ b)) : whn Γ (a → b) :=
  RamLec (t_rename (r_shift s_pop) (t+ _) u) . 

Definition ty_nat (a : ty0) : ty0 := (a → ((a → a) → a)) .
Definition nat_ze {a} : term ∅ (t+ ty_nat a) :=
  Val (RamLec (Val (RamLec (Val (VarP (pop (pop top))))))) .
Definition nat_su {a} : term ∅ (t+ ty_nat a) :=
  Val (Ram' (Val (Ram' (App' (Val (VarP top)) (VarP (pop top)))))).

Definition nat_sum {a} : term ∅ (t+ (ty_nat a → (ty_nat a → ty_nat a))) .
  do 4 refine (Val (Ram' _)).
...
with w_subst {Γ Δ} : Γ =[val]> Δ -> whn Γ ⇒ᵢ val Δ :=
  ...
  w_subst f _ (ArrR c)     := Whn (ArrR (s_subst (a_shift2 f) c)) ;
  w_subst f _ (ArrL v k) := ArrL (w_subst f _ v) (w_subst f _ k) ;
...
Definition ass1 {Γ a} (v : val Γ a) : (Γ ▶ a) =[val]> Γ := s_append s_var v .
...
| PArr {A B} : pat (t+ A) -> pat (t- B) -> pat (t- (A → B))
...
Equations p_dom {t} : pat t -> neg_ctx :=  
  ...
  p_dom (PArr p1 p2) := p_dom p1 +▶ₛ p_dom p2 ;
...
Equations w_of_p {a} (p : pat a) : whn (p_dom p) a :=
  ...
  w_of_p (PArr p1 p2) := ArrL (w_rename r_concat_l _ (w_of_p p1)) (w_rename r_concat_r _ (w_of_p p2)) ;
...
Equations p_of_w_0p {Γ : neg_ctx} (A : ty0 pos) : whn Γ (t+ A) -> pat (t+ A) :=
  p_of_w_0p (B+) (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
...
with p_of_w_0n {Γ : neg_ctx} (A : ty0 neg) : whn Γ (t- A) -> pat (t- A) :=
  p_of_w_0n (B-) (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
  p_of_w_0n (A → B) (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
  p_of_w_0n (↑ A)  (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
  p_of_w_0n (A → B) (ArrL v k) := PArr (p_of_w_0p A v) (p_of_w_0n B k) ;
...
Equations p_dom_of_w_0p {Γ : neg_ctx} (A : ty0 pos) (v : whn Γ (t+ A)) : p_dom (p_of_w_0p A v) =[val]> Γ by struct A :=
  p_dom_of_w_0p (B+) (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
...
with p_dom_of_w_0n {Γ : neg_ctx} (A : ty0 neg) (k : whn Γ (t- A)) : p_dom (p_of_w_0n A k) =[val]> Γ by struct A :=
  p_dom_of_w_0n (B-) (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
  p_dom_of_w_0n (A → B) (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
  p_dom_of_w_0n (↑ A)  (Var i) with (s_elt_upg i).(sub_prf) := { | ! } ;
  p_dom_of_w_0n (A → B) (ArrL v k) := [ p_dom_of_w_0p A v , p_dom_of_w_0n B k ] ;
  p_dom_of_w_0n (↑ A)  (ShiftNL x)  := s_append s_empty x .

Equations p_dom_of_v {Γ : neg_ctx} A (v : val Γ A) : p_dom (p_of_v A v) =[val]> Γ :=
  p_dom_of_v (@VTy pos A) v := p_dom_of_w_0p A v ;
  p_dom_of_v (@KTy pos A) x := s_append s_empty x ;
  p_dom_of_v (@VTy neg A) x := s_append s_empty x ;
...
Equations eval_aux {Γ : neg_ctx} : state Γ -> (state Γ + nf Γ) :=
  ...
  eval_aux (Cut neg (Whn (ArrR c))     (Whn (ArrL v k))) := inl (c ₛ/[ v , k ]) ;
...
Record syn_ind_args
  ...
  (P_s : forall Γ, state Γ -> Prop) :=
  {
    ...
    ind_arrr : forall Γ A B s (H : P_s _ s), P_w Γ (t+ (A → B)) (ArrR s) ;
    ind_arrl : forall Γ A B w1 (H1 : P_w _ _ w1) w2 (H2 : P_w Γ (t- B) w2), P_w Γ (t- (A → B)) (ArrL w1 w2) ;
...
Lemma refold_id_aux {Γ p} A : refold_id_aux_P Γ p A .
  ...
  - dependent elimination v.
    ...
    change (w_subst _ _ _) with
      (w_subst ([ p_dom_of_w_0p A3 w , p_dom_of_w_0n B0 w0 ]) _
         (ArrL (w_rename r_concat_l _ (w_of_p (p_of_w_0p A3 w)))
               (w_rename r_concat_r _ (w_of_p (p_of_w_0n B0 w0))))); cbn.
    rewrite 2 w_sub_ren.
  ...
  - dependent elimination v.
    pose (nope := (s_elt_upg h).(sub_prf)); dependent elimination nope.
    exact (f_equal OrR1 (IHA1 w1)).
    exact (f_equal OrR2 (IHA2 w2)).
  - dependent elimination v.
    pose (nope := (s_elt_upg h).(sub_prf)); dependent elimination nope.
    reflexivity.
  - dependent elimination v.
...
Lemma clean_hyp {Γ Δ : neg_ctx} (c : state Γ) (e : Γ ⊆ Δ)
  ...
  unfold it_eq; revert Γ c e; coinduction L CIH; intros Γ c e.
  cbn.
  ...
  - dependent elimination t1.
    + cbn; econstructor. unfold n_rename; cbn.
      ...
      cbv [observe]; destruct (_observe (eval (Cut (Val (w_subst e a2 v)) (e (t- a2) h)))); econstructor; reflexivity.
    + unfold eval. cbn -[then_eval2]; change (iter _ T1_0 ?x) with (eval x).
      change (w_subst e a3 v) with (v_subst e (t+ a3) v); rewrite s_sub1_sub.
    ...
    + dependent elimination v. (* Cut (Val _) (App _ _)) *)
      * unfold eval at 2; cbn -[eval then_eval2].
        ...
        rewrite (w_sub_eq _ _ H a4 _ _ eq_refl), refold_id.
        cbn -[eval_aux];
          destruct (eval_aux (Cut (Val (e _ h)) (App (w_subst e _ v1) (t_subst e _ t0))));
          econstructor;
          reflexivity.
      * cbn; econstructor; change (iter _ T1_0 ?x) with (eval x).
        change (RamLec (t_subst (a_shift2 e) (t+ b2) t1)) with (w_subst e (a2 → b2) (RamLec t1)).
        unfold t_subst1 at 1 2.
        rewrite 2 t_sub_sub.
        rewrite <- (t_sub_eq _ _ (a_comp_assoc _ _ _) (t+ b2) t1 t1 eq_refl).
        rewrite <- t_sub_sub.
        unfold w_shift at 1; rewrite w_ren_sub.
        ...
        ** intros ? i; dependent elimination i as [ Ctx.top | Ctx.pop Ctx.top | Ctx.pop (Ctx.pop i) ]; cbn.
           ++ rewrite w_sub_ren; now apply w_sub_eq.
           ++ reflexivity.
           ++ destruct x1; cbn.
              rewrite w_ren_ren, w_sub_ren.
              rewrite <- (w_sub_id_l t (w_rename s_pop t (e _ i))), w_sub_ren.
              now apply w_sub_eq.
        ...
        ** rewrite (t_sub_eq _ _ H _ _ _ eq_refl).
        ...
        rewrite (t_sub_eq _ _ (a_comp_assoc _ _ _) (t+ b2) t1 t1 eq_refl).
        rewrite <- t_sub_sub.
        change (w_subst e _ (RamLec t1)) with (v_subst e (t+ _) (RamLec t1)).
        rewrite (t_sub_eq _ _ (sub1_sub (a:=t+ _) e (RamLec t1)) _ _ _ eq_refl).
        rewrite <- t_sub_sub.
    ...
    + dependent elimination v. (* Cut (Val _) (Match _ _) *)
      * pose (nope := (s_elt_upg h).(sub_prf)); dependent elimination nope.
      * cbn; econstructor; change (iter _ T1_0 ?x) with (eval x).
        change (w_subst e a0 v) with (v_subst e (t+ a0) v); rewrite s_sub1_sub.
        apply CIH.
      * cbn; econstructor; change (iter _ T1_0 ?x) with (eval x).
        change (w_subst e b0 v0) with (v_subst e (t+ b0) v0); rewrite s_sub1_sub.
        apply CIH.
Qed.
  - 
    unfold 
    ...
    + dependent elimination t1.
      ...
      * dependent elimination w.
        ...
        -- dependent elimination w0.
           ++ unfold eval at 2; cbn -[eval then_eval2].
              unfold then_eval2; cbn -[eval eval_aux].
              simp eval_aux; cbn [fst snd projT1 projT2].
              rewrite (refold_id_aux (A3 ⊕ B2)).
        ...
        -- dependent elimination w0.
           ++ unfold eval at 2; cbn -[eval then_eval2].
              unfold then_eval2; cbn -[eval eval_aux].
              simp eval_aux; cbn [fst snd projT1 projT2].
              rewrite (refold_id_aux (↓ A6)).
              now change (it_eqF _ ?RX ?RY _ (observe ?a) (_observe ?b)) with (it_eq_map ∅ₑ RX RY T1_0 a b).
           ++ cbn; econstructor.
              change (Whn (ShiftPL (s_subst _ _))) with (t_subst e (t- _) (Whn (ShiftPL s2))).
      ...
      * dependent elimination w.
        ...
        -- dependent elimination w0.
           ++ unfold eval at 2; cbn -[eval then_eval2].
              unfold then_eval2; cbn -[eval eval_aux].
              simp eval_aux; cbn [fst snd projT1 projT2].
              rewrite (refold_id_aux (A2 → B1)).
              now change (it_eqF _ ?RX ?RY _ (observe ?a) (_observe ?b)) with (it_eq_map ∅ₑ RX RY T1_0 a b).
           ++ cbn; econstructor.
              change (w_subst ?f ?A ?w) with (v_subst f A w) at 1 2.
              rewrite s_sub2_sub; apply CIH.
        -- dependent elimination w0.
           ++ unfold eval at 2; cbn -[eval then_eval2].
              unfold then_eval2; cbn -[eval eval_aux].
              ...
              now change (it_eqF _ ?RX ?RY _ (observe ?a) (_observe ?b)) with (it_eq_map ∅ₑ RX RY T1_0 a b).
           ++ cbn; econstructor.
              change (Whn (ShiftNR (s_subst _ _))) with (t_subst e (t+ _) (Whn (ShiftNR s3))).
              change (t_subst ?f ?A ?w) with (v_subst f A w).
              rewrite s_sub2_sub; apply CIH.
Qed.
...
Equations p_of_w_eq_aux_p {Γ : neg_ctx} (A : ty0 pos) (p : pat (t+ A)) (e : p_dom p =[val]> Γ)
  ...
  p_of_w_eq_aux_p (↓ A)  (PShiftP _)  e := eq_refl ;
     ...
     with p_of_w_eq_aux_n {Γ : neg_ctx} (A : ty0 neg) (p : pat (t- A)) (e : p_dom p =[val]> Γ)
         ...
         by struct A :=
  p_of_w_eq_aux_n (A → B) (PArr p1 p2) e := _ ;
  p_of_w_eq_aux_n (↑ A)  (PShiftN _)  e := eq_refl .
Obligation 1.
  rewrite 2 w_sub_ren.
  refine (f_equal2 PArr _ _); [ apply p_of_w_eq_aux_p | apply p_of_w_eq_aux_n ].
Defined.
...
Lemma p_dom_of_v_eq {Γ p} A : p_dom_of_w_eq_P Γ p A . 
  ...
  - unfold p_of_w_eq_aux_p_obligations_obligation_1.
    ...
    pose (H2 := p_of_w_eq_aux_n B0 p0 (e ⊛ᵣ r_concat_r)); change (p_of_w_eq_aux_n B0 _ _) with H2 in IHA2 |- *.
    transitivity ([ rew [fun p : pat (t+ A3) => p_dom p =[ val ]> Γ] H1
                    ...
                    rew [fun p : pat (t- B0) => p_dom p =[ val ]> Γ] H2
                     in p_dom_of_w_0n B0 (w_subst (e ⊛ᵣ r_concat_r) (t- B0) (w_of_p p0)) ]).
...
Lemma eval_nf_ret {Γ : neg_ctx} (u : nf Γ) : eval (p_app (s_var _ (fst (projT2 u))) (projT1 (snd (projT2 u))) (projT2 (snd (projT2 u)))) ≋ ret_delay u .
  ...
  - simpl_depind.
    ...
    apply (p_of_w_eq_aux_n _ (PArr p1 p2)).
    apply (p_dom_of_v_eq (_ → _) (PArr p1 p2)).
  - simpl_depind.
    destruct A; inversion eqargs; cbn in eqargs.
...
Lemma eval_app_not_var : well_founded head_inst_nostep .
  ...
  - dependent elimination w.
    * apply it_eq_step in H; cbn in H.
      ...
      revert m i H2; rewrite <- H1; intros m i H2.
      inversion_clear H2; cbn; clear.
      econstructor; intros [ A p1 ] H; dependent elimination H; cbn in *.
      dependent elimination v.
      + apply it_eq_step in H; now inversion H.
      + dependent elimination w.
