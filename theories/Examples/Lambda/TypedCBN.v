...
| Var {a} : Γ ∋ + a -> term Γ a
| Lam {a b : ty0} : term (Γ ▶ a) b -> term Γ (a → b)
...
| K0 {a} : Γ ∋ ¬ a -> ev_ctx Γ a
| K1 {a b} : term Γ a -> ev_ctx Γ b -> ev_ctx Γ (a → b)
.
(*|
.. coq:: none
|*)
Arguments K0 {Γ a} i .
Arguments K1 {Γ a b} u π .
(*|
...
Equations t_rename {Γ Δ} : Γ ⊆ Δ -> term Γ ⇒ᵢ term Δ :=
  ...
  t_rename f _ (Lam t)     := Lam (t_rename (r_shift f) _ t) ;
...
Equations e_rename {Γ Δ} : Γ ⊆ Δ -> ev_ctx Γ ⇒ᵢ ev_ctx Δ :=
  ...
  e_rename f _ (K1 u π) := K1 (t_rename f _ u) (e_rename f _ π) .
...
Equations t_subst {Γ Δ} : Γ =[val_m]> Δ -> term Γ ⇒ᵢ term Δ :=
  ...
  t_subst f _ (Lam t)     := Lam (t_subst (a_shift f) _ t) ;
...
Equations e_subst {Γ Δ} : Γ =[val_m]> Δ -> ev_ctx Γ ⇒ᵢ ev_ctx Δ :=
  ...
  e_subst f _ (K1 v π) := K1 (t_subst f _ v) (e_subst f _ π) .
...
Equations obs_app {Γ a} (v : val_m Γ a) (p : obs a) (γ : obs_dom p =[val_m]> Γ) : state Γ :=
  obs_app v (OApp) γ := Cut v (K1 (γ _ (pop top)) (γ _ top)) ;
  obs_app π (OLam) γ := Cut (γ _ top) π .
(*|
Normal forms
^^^^^^^^^^^^
